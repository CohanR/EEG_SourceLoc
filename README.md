...
<i>Example of a decent coregistration and correct fiducial coordinates</i>
</div>
...
Source Time Course and ROI Extraction: At this stage, cortical activation time courses are computed for all vertices in the source space. This covers the entire range of channels (electrodes). One can only focus on one or a set of ROIs.  For examplpe if you wish to only look at the timeseries extracted from the left and right motor cortices, you can defined the ROI coordinates using Freesurfer parcellations, and then extract their mean activation time courses (that were previously determined using forward and inverse modelling). This step enables region-specific analyses, such as identifying peak activations in the left and right hemispheres. But agin, if you wish to look at the entire brain, defining ROIs in this sense would be redundant. 
